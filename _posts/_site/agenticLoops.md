# The Calculus of Self-Correction: Why Agentic Loops are Dynamical Systems
...
In metric space terms, if we have two different potential plans $z_a$ and $z_b$, applying the reflection operator should, on average, bring them closer to the "correct" fixed point $z^*$. Mathematically, we’re looking for a Lipschitz constant $k < 1$ such that:

$$d(R(z_a), R(z_b)) \leq k \cdot d(z_a, z_b)$$

When this condition is met, the **Banach Fixed-Point Theorem** guarantees that the agent will eventually converge to a stable, successful plan, regardless of its initial "System 1" guess. If $k \geq 1$, you get an agent that "overthinks" itself into nonsense—a phenomenon we see daily in unoptimized agentic loops.
...
For a stable agent, each loop of reflection must result in a decrease in this energy:

$$\mathbb{E}[V(z_{t+1}) - V(z_t) | z_t] < 0$$
