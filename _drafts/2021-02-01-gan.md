...
In short, a generative model is a statistical model of the joint probability $$P(X,Y)$$ on given observed variable $$X$$ and target variable $$Y$$. Here we have this generative approach for modeling the class-conditional densities $$p(\mathbf{x}|C_k)$$, as well as the class priors $$p(C_k)$$, then we use them to compute posterior probabilities $$p(C_k|\mathbf{x})$$ using Baye's Theorem.
Consider first of all the case of two classes. Then the posterior probability for class $$C_1$$ can be written as
...
where
* $$a$$ is defined as $$\log(p(\mathbf{x}\>|\>C_1)p(C_1))/p(\mathbf{x}\>|\>C_2)p(C_2)$$.
* $$\sigma$$ is the *logistic sigmoid function*.
Additionally, the logit function, the inverse of the sigmoid function is given by

$$a=\log\left(\frac{\sigma}{1-\sigma}\right)$$.

For number of classes $$K>2$$, we have
...
This is known as the *normalized exponential*, and can be regarded as a multiclass generalization of the sigmoid function. Here the quantities $$a_k$$'s are defined by 

$$a_k=\log \>p(\mathbf{x}\>|\>C_k)p(C_k)$$

Now, consider the discrete feature values $$x_i$$. For simplicity we will assume the features to be binary $$x_i\in\{0,1\}$$. Suppose we have $$D$$ inputs, then a general distribution will correspond to a table of $$2^D$$ numbers for each class with $$2^D-1$$ independent variables (due to the summation constraint).

We will make a *naive Bayes* assumption, where the feature values are treated independent and conditioned on the class $$C_k$$. Then we have class-conditional distributions of the form

$$p(\mathbf{x}\>|\>C_k)=\prod^D_{i=1}\mu_{ki}^{x_i}(1-\mu_{ki})^{1-x_i}$$
